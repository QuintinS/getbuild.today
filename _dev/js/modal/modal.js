BuildModal = function(options) {
	...
	var $this = this,
		...
		currentBodyContent = "",
		sidebarOpen = false,
		templateModal = $options.modaltemplate || '<div class="modal" id="buildModal"></div><!-- /.modal -->';

	var $defaults = {
		animateIn: "zoomIn",
		animateOut: "fadeOutUp",
		trip: null, // null or trip.js Trip object.
		backdrop: false,
		bsoptions: {
			backdrop: true,
			keyboard: true,
		...
		if (typeof $options === "object") {
			...
			if (typeof $options.content === "object") {
				...
				else
				{
					...
					if ((typeof $options.content.body === "object")) {
						contentType = "object";
	...
	this.breadcrumbs = {
		...
		init : function () {
			...
			for (var i = 0; i <= $breadcrumbs.length - 1; i++) {
				if (i < $breadcrumbs.length - 1) {
					returnHTML += printBreadcrumb({type:"link", text:$breadcrumbs[i].title});
					returnHTML += printBreadcrumbDivider();
				}
				else
				{
					returnHTML += printBreadcrumb({type:"final", text:$breadcrumbs[i].title});
				}
			}
		...
		add : function(sectionTitle) {
			...
			$breadcrumbs.push({title: sectionTitle});
		...
		reset : function() {
			$breadcrumbs = [{title: $options.header.title}];
			return $breadcrumbs;
		},
	...
	this.initSidebar = function() {
		...
		for (var i = 0; i < contentArray.length; i++) {
			...
			myOptions 	= 	{ isActive: myActive, icon: myIcon, title: myTitle, badge: myBadge, actions: myActions };
	...
	this.show = function() {
		...
		// $("#buildModal > .modal-backdrop").addClass("animated");
		// $("#buildModal > .modal-backdrop").addClass("fadeIn");
	};
	...
	this.hide = function() {
		...
		// $("#buildModal").addClass("animated");
		// $("#buildModal").addClass(this.animateOut);
	};
	...
	this.load = function(options) {
		...
		*/
		...
		if (options.hasOwnProperty("body") === true) {
	...
	this.printButton = function(options) {
		...
		defaults = {
			...
			icon: "", // Icon Classes
			actions: "", // Function Event Listener Names 
			type: "button", // ONLY button || submit
			classes: "btn btn-primary", // Extra classes
